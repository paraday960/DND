    ...
    for attempt in range(retries + 1):
        try:
            headers = {
                ...
                "User-Agent": "DND-Bot-Android/2.54",
                "Connection": "close",
            }
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def _user(self):
        ...
        try:
            ...
            pass
        # حالت مهمان (وب‌محور): guest_id+guest_name از query/header/body
        try:
            q = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
            gid = ((q.get("guest_id") or [None])[0]
                   or self.headers.get("X-Guest-Id")
                   or "")
            gname = ((q.get("guest_name") or [None])[0]
                     or self.headers.get("X-Guest-Name")
                     or "")
            if not gid or not gname:
                b = self._body() or {}
                gid = gid or str(b.get("guest_id", "") or "")
                gname = gname or str(b.get("guest_name", "") or "")
            if gid and gname:
                try:
                    gid_int = int(gid)
                except (TypeError, ValueError):
                    gid_int = 900000000 + (abs(hash(str(gid))) % 99000000)
                clean = (gname or "میهمان").strip()[:30] or "میهمان"
                return {"id": gid_int, "first_name": clean, "_guest": True}
        except Exception:
            pass
        return None
    ...
    def do_POST(self):
        ...
        user = self._user()
        if not user:
            self._json(401, {"ok": False, "error": "شناسایی نشدی — لطفاً نام خود را وارد کن تا وارد شوی."})
            return
        body = self._body()
        code = (body.get("room") or body.get("code") or "").strip().upper()
...
def _kill_old_cloudflareds():
    """پروسه‌های cloudflared باقی‌مانده از اجرای قبلی را می‌کشد تا port/edge قفل نباشد."""
    try:
        os.system(
            "pkill -9 cloudflared 2>/dev/null; "
            "pkill -9 -f 'libcloudflared' 2>/dev/null; "
            "pkill -9 -f 'cf_config.yml' 2>/dev/null; "
            "pkill -9 -f 'trycloudflare' 2>/dev/null; "
            "for p in /proc/[0-9]*/cmdline; do "
            "  if [ -r \"$p\" ] && grep -aq cloudflared \"$p\" 2>/dev/null; then "
            "    kill -9 $(echo $p | cut -d/ -f3) 2>/dev/null; "
            "  fi; "
            "done; true")
        time.sleep(1.5)
    except Exception:
            ...
            try:
                ...
                for i in range(20):
                    ...
                    try:
                        req = urllib.request.Request(
                            url.rstrip("/") + "/healthz",
                            headers={"User-Agent": "DND-Bot-hc/2.54", "Connection": "close"})
                ...
                while not stopped() and proc.poll() is None:
                    ...
                    try:
                        req = urllib.request.Request(url.rstrip("/") + "/healthz",
                            headers={"User-Agent": "DND-Bot-check/2.54", "Connection": "close"})
                        with urllib.request.urlopen(req, timeout=6) as r:
                            body = r.read(200)
                    ...
                    if not ext_ok:
                        ext_fail += 1
                        # فقط اولین بار و هر ۱۰ دقیقه یک‌بار لاگ بده تا لاگ اسپم نشود (هر ۸ ثانیه ۷۵ بار در ۱۰ دقیقه)
                        if ext_fail == 1 or ext_fail % 75 == 0:
                            log("ℹ️ healthz از بیرون پاسخ نداد (hairpin NAT روی موبایل طبیعی است).")
                        # اگر اتصال CF واقعاً از بین رفت (نه صرفاً hairpin)، ری‌استارت
